...
class RunCommandTool : BaseMcpTool() {
    ...
    override fun getDescription(): String = "Runs a shell command in the terminal environment. PREFER NATIVE MCP TOOLS instead: readFile/cat for reading, searchCode/findInFiles/grep for search, findFiles/glob for file find, batchReplace/editFile for editing, diffFiles for comparing, head/tail/wc/stat for file info, listFiles/ls for directory listing. Only use runCommand for compiling, running, or package installs that have no native MCP tool."
